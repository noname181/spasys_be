...
class RateMetaDataController extends Controller
{
    ...
    public function getAllRM(RateMetaDataSearchRequest $request)
    {
        ...
        try {
            ...
            $rmd = RateMetaData::with(['rate_meta', 'member:mb_no,co_no'])->whereNotNull('rm_no')->orderBy('rmd_no', 'DESC');
    ...
    public function getAllCO(RateMetaDataSearchRequest $request)
    {
        ...
        try {
            ...
            // If page is null set default data = 1
            $page = isset($validated['page']) ? $validated['page'] : 1;
            $rmd = RateMetaData::with(['rate_meta', 'member:mb_no,co_no'])->whereNotNull('co_no')->orderBy('rmd_no', 'DESC');
            if(isset($validated['from_date'])) {
                $rm->where('created_at', '>=' , date('Y-m-d 00:00:00', strtotime($validated['from_date'])));
            }
            if(isset($validated['to_date'])) {
