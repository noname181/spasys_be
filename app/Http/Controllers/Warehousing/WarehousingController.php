...
class WarehousingController extends Controller
{
...

    {
        try {
            ...
            $user = Auth::user();
            if ($user->mb_type == 'shop') {
                $warehousing = ReceivingGoodsDelivery::with(['w_no', 'warehousing'])->with(['mb_no'])->join('warehousing', 'warehousing.w_no', '=', 'receiving_goods_delivery.w_no')->whereHas('w_no', function ($query) use ($user) {
            ...
            if ($validated['service'] == "유통가공") {
                ...
                if ($user->mb_type == 'shop') {
                    ...
                    $sub = ImportExpected::select('company.co_type', 't_import_expected.tie_status_2 as import_expected', 'parent_spasys.co_name as co_name_spasys', 'parent_spasys.co_no as co_no_spasys', 'parent_shop.co_name as co_name_shop', 'parent_shop.co_no as co_no_shop', 'company.co_no', 'company.co_name', 't_import_expected.*')
                        ...
                        $sub_2 = Import::select('receiving_goods_delivery.rgd_no','receiving_goods_delivery.rgd_confirmed_date','receiving_goods_delivery.rgd_status3','receiving_goods_delivery.rgd_tracking_code','receiving_goods_delivery.rgd_delivery_company','receiving_goods_delivery.rgd_address', 'receiving_goods_delivery.rgd_status1','ti_status_2', 'ti_logistic_manage_number', 'ti_i_confirm_number', 'ti_i_date', 'ti_i_order', 'ti_i_number', 'ti_carry_in_number')
...

    {
        try {
            ...
            if ($user->mb_type == 'shop') {
                ...
                $warehousing = ReceivingGoodsDelivery::with(['w_no', 'warehousing'])->with(['mb_no'])->whereNotIn('w_no', $w_import_no)->whereHas('w_no', function ($query) use ($user) {
...

    {
        try {
            ...
            if ($user->mb_type == 'shop' && $request->type == 'check_list') {
                $warehousing = ReceivingGoodsDelivery::with(['mb_no','rate_meta_data', 'w_no', 'rate_data_general','payment'])->whereHas('w_no', function ($query) use ($user) {
                    $query->whereHas('co_no.co_parent', function ($q) use ($user) {
                        $q->where('co_no', $user->co_no);
                    });
                })->whereHas('mb_no', function ($q) {
            ...
            if ($user->mb_type == 'shop' && $request->type == 'view_list') {
                $warehousing_fulfillment = ReceivingGoodsDelivery::with(['mb_no', 'w_no', 'rate_data_general'])->whereHas('w_no', function ($query) use ($user) {
                    $query->whereHas('co_no.co_parent', function ($q) use ($user) {
                        $q->where('co_no', $user->co_no);
                    });
                });
            } else if ($user->mb_type == 'shop' && $request->type == 'check_list') {
                $warehousing_fulfillment = ReceivingGoodsDelivery::with(['mb_no','rate_meta_data', 'w_no', 'rate_data_general'])->whereHas('w_no', function ($query) use ($user) {
                    $query->whereHas('co_no', function ($q) use ($user) {
                        $q->where('co_no', $user->co_no);
                    });
                });
            ...
            if (isset($validated['co_name'])) {
                $warehousing_fulfillment->whereHas('w_no.co_no', function ($q) use ($validated) {
                    return $q->where(DB::raw('lower(co_name)'), 'like', '%' . strtolower($validated['co_name']) . '%');
                });
            }
            if (isset($validated['w_type'])) {
                $warehousing_fulfillment->whereHas('w_no', function ($q) use ($validated) {
                    return $q->where('w_type', 'like', '%' . $validated['w_type'] . '%');
                });
            }
            if (isset($validated['w_schedule_number'])) {
            ...
            if (isset($validated['rgd_status3'])) {
                $warehousing_fulfillment->where('rgd_status3', '=', $validated['rgd_status3']);
            }
            if (isset($validated['rgd_status3_1']) || isset($validated['rgd_status3_2']) || isset($validated['rgd_status3_3'])) {
            ...
            if ($user->mb_type == 'shop' && $request->type == 'view_list') {
                ...
                $warehousing_bonded = ReceivingGoodsDelivery::with(['mb_no','rate_meta_data', 'w_no', 'rate_data_general', 't_export'])->whereHas('w_no', function ($query) use ($user) {
                    $query->whereHas('co_no.co_parent', function ($q) use ($user) {
                        $q->where('co_no', $user->co_no);
                    });
                })->whereHas('mb_no', function ($q) use ($user) {
            ...
            if (isset($validated['co_parent_name'])) {
                $warehousing_bonded->whereHas('w_no.co_no.co_parent', function ($query) use ($validated) {
                    $query->where(DB::raw('lower(co_name)'), 'like', '%' . strtolower($validated['co_parent_name']) . '%');
                });
            }
            if (isset($validated['co_name'])) {
                $warehousing_bonded->whereHas('w_no.co_no', function ($q) use ($validated) {
                    return $q->where(DB::raw('lower(co_name)'), 'like', '%' . strtolower($validated['co_name']) . '%');
                });
            }

            if (isset($validated['settlement_cycle'])) {
                $warehousing_bonded->whereHas('w_no.co_no.company_bonded_cycle', function ($q) use ($validated) {
                    return $q->where('cs_payment_cycle', $validated['settlement_cycle']);
                });
            }

            if (isset($validated['w_type'])) {
                $warehousing_bonded->whereHas('w_no', function ($q) use ($validated) {
                    return $q->where('w_type', 'like', '%' . $validated['w_type'] . '%');
                });
            }
            if (isset($validated['w_schedule_number'])) {
            ...
            if (isset($validated['rgd_status3'])) {
                $warehousing_bonded->where('rgd_status3', '=', $validated['rgd_status3']);
            }
...

    {
        try {
            ...
            $user = Auth::user();
            if ($user->mb_type == 'shop') {
                $warehousing = ReceivingGoodsDelivery::with(['mb_no', 'w_no', 'rate_data_general', 't_import_expected', 't_import', 't_export', 'rate_meta_data' => function ($q) {
