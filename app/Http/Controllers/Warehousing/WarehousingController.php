...
class WarehousingController extends Controller
{
...

    {
        try {
            ...
            $warehousing = ReceivingGoodsDelivery::with(['rate_meta_data', 'rate_meta_data_parent', 'rate_data_general', 'payment', 't_import', 'rgd_settlement']);
            ...
            $warehousing_bonded = ReceivingGoodsDelivery::with(['rate_meta_data', 'rate_data_general', 't_export', 'rgd_settlement']);
            if ($user->mb_type == 'shop' && $request->type == 'view_list') {
            ...
            if (isset($validated['settlement_cycle'])) {
                $warehousing_bonded->whereHas('warehousing.co_no.company_bonded_cycle', function ($q) use ($validated) {
                    return $q->where('cs_payment_cycle', $validated['settlement_cycle']);
                });
            }
        ...
        try {
            ...
            if (isset($validated['rgd_status6'])) {
                if ($validated['rgd_status6'] == 'paid') {
                    $warehousing->where('rgd_status6', '=', $validated['rgd_status6']);
                } else if ($validated['rgd_status6'] == '전체') {
                   
                } else
                    $warehousing->whereNull('rgd_status6');
            }
