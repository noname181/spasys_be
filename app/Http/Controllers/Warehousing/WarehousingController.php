        ...
        try {
            ...
            if ($validated['service'] == "유통가공") {
                ...
                $final3 = collect($import_schedule)->map(function ($q) {
                    if(isset($q->te_carry_out_number)){
                        $package = Package::where('w_no', $q->te_carry_out_number)->first();
                    } else if(isset($q->ti_carry_in_number) && $q->te_carry_out_number == null) {
                        $package = Package::where('w_no', $q->ti_carry_in_number)->first();
                    } else if(isset($q->tie_logistic_manage_number) && $q->ti_carry_in_number == null && $q->te_carry_out_number == null){
                        $package = Package::where('w_no', $q->tie_logistic_manage_number)->first();
                    }
...

    {
        try {
            ...
            if ($user->mb_type == 'shop') {
                ...
                })->with(['tax_invoice', 'payment', 'member', 'warehousing', 'rate_data_general', 't_export', 't_import'])->whereHas('warehousing', function ($query) use ($user) {
                ...
                $warehousing->getCollection()->map(function ($item) {
                    if (isset($item['rate_data_general']))
                        if ($item->service_korean_name === "유통가공") {
                            ...
                            $item->supply_price_total = $item->rate_data_general->rdg_supply_price7 ? ($item->rate_data_general->rdg_supply_price7 + ($item->rate_data_general->rdg_supply_price14 ? $item->rate_data_general->rdg_supply_price14 : 0)) : 0;
                            $item->vat_price_total = $item->rate_data_general->rdg_vat7 ? ($item->rate_data_general->rdg_vat7 + ($item->rate_data_general->rdg_vat14 ? $item->rate_data_general->rdg_vat14 : 0)) : 0;
                            $item->sum_price_total = $item->rate_data_general->rdg_sum7 ? ($item->rate_data_general->rdg_sum7 + ($item->rate_data_general->rdg_sum14 ? $item->rate_data_general->rdg_sum14 : 0)) : 0;
                        }
        ...
        try {
            ...
            // }
                ...
                foreach ($request->rgds as $rgd) {
                    ...
                    $rgd = ReceivingGoodsDelivery::with(['warehousing', 'rate_data_general', 't_import_expected', 't_import'])->where('rgd_no', $rgd['rgd_no'])->first();
        ...
        if ($rgds) {
            foreach ($rgds as $rgdp) {
                ...
                if ($rgdp['service_korean_name'] == '보세화물') {
                    if (str_contains($rgdp['rgd_bill_type'], 'month')) {
                        ...
                        $description = $rgdp['t_import']['ti_h_bl'] . ($count == 1 ? '' : ('외 ' . ($count - 1) . '건'));
                    } else {
                        $item_name = $rgdp['t_import']['ti_h_bl'];
                        $description = $rgdp['t_import']['ti_h_bl'];
                    }
                    $amount_price += ($rgdp['rate_data_general']['rdg_supply_price7'] + $rgdp['rate_data_general']['rdg_supply_price14']);
                    $total_price += ($rgdp['rate_data_general']['rdg_sum7'] + $rgdp['rate_data_general']['rdg_sum14']);
                    $vat_price += ($rgdp['rate_data_general']['rdg_vat7'] + $rgdp['rate_data_general']['rdg_vat14']);
                    ...
                    if (str_contains($rgdp['rgd_bill_type'], 'month')) {
                        $count = ReceivingGoodsDelivery::where('rgd_settlement_number', $rgdp['rgd_settlement_number'])->count();
                        $item_name = $rgdp['rgd_settlement_number'];
                        $description = $rgdp['warehousing']['w_schedule_number2'] . ($count == 1 ? '' : ('외 ' . ($count - 1) . '건'));
                    } else {
                        $item_name = $rgdp['rgd_settlement_number'];
                        $description = $rgdp['warehousing']['w_schedule_number2'];
                    }
                    $amount_price += $rgdp['rate_data_general']['rdg_supply_price4'];
                    $total_price += $rgdp['rate_data_general']['rdg_sum4'];
                    $vat_price += $rgdp['rate_data_general']['rdg_vat4'];
            ...
            if ($rgd['service_korean_name'] == '보세화물') {
                if (str_contains($rgd['rgd_bill_type'], 'month')) {
                    ...
                    $description = $rgd['t_import']['ti_h_bl'] . ($count == 1 ? '' : ('외 ' . ($count - 1) . '건'));
                } else {
                    $item_name = $rgd['rgd_settlement_number'];
                    $description = $rgd['t_import']['ti_h_bl'];
                }
                ...
                if (str_contains($rgd['rgd_bill_type'], 'month')) {
                    $count = ReceivingGoodsDelivery::where('rgd_settlement_number', $rgd['rgd_settlement_number'])->count();
                    $item_name = $rgd['rgd_settlement_number'];
                    $description = $rgd['warehousing']['w_schedule_number2'] . ($count == 1 ? '' : ('외 ' . ($count - 1) . '건'));
                } else {
                    $item_name = $rgd['rgd_settlement_number'];
                    $description = $rgd['warehousing']['w_schedule_number2'];
                }
            }
